...
$(function () {
    ...
    function initUI(){    
        ...
        //部件详情
        $('#btn_details').on(startEventType, function(){
            var _selected = viewer.OperatePlugin.getSelected();
            if(_selected){
                if(hasDetailParts.indexOf(_selected.name) > -1){
                    $('#detailsViewer').attr('src', 'iframe.html?mn=' + _selected.name);
                    $('#detailsViewer').show();
                    $('#btn_back').show();
                }else{
                    alert('无此部位细节模型!');
        ...
        $('#btn-test').on(startEventType, function() {
            var testObj = viewer.getObjectByName('弓头组装');
            if(testObj){
                viewer.OperatePlugin.transparentOthers(testObj);
            }
        });
    }
