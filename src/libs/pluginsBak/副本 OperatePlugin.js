        ...
        curSelectObj.traverse(function(m){
            if(m.name !== 'shadow' && m.material){
                if(m.material instanceof Array){
                    ...
                    m.material = cloneMatCache[mat.name];
                }
            }
        });
    }
    ...
    if(mainObj){  
            ...
            shadow.parent.children.forEach(function(obj){
                if(obj !== shadow){
                    obj.children.forEach(function(o){
                        allModels.push(o);
                        bakPositions[oid] = o.position.clone();
                    });                   
                }
            });
        }else{
        ...
        dom.addEventListener(startEventType, function(e) {
            if(_operating && !canDrag){
                ...
                var intersects = raycaster.intersectObjects(allModels);
                if(intersects.length > 0){                
                    if (curSelectObj != intersects[0].object) {
                        //设置选中效果
                        setSelect(function(){
                            curSelectObj = intersects[0].object;
                        });
                    
                        if(canReset){
                            var hitObj = intersects[0].object;
                            hitObj.position.copy(bakPositions[hitObj.id]);
                        }
                    }
                }
            }
        ...
        Util.extend(viewer[this.name], {
            ...
            transparent:function(){
                if(curSelectObj){
                    ...
                    restoreMat();
                    //透明或恢复透明
                    var flag = curSelectObj.material instanceof Array;                
                    if(hasTransparent(flag ? curSelectObj.material[0] : curSelectObj.material)){
                        if(flag){
                            curSelectObj.material.forEach(function(mat){
                                unTransparent(mat);
                            });
                        }else{
                            unTransparent(curSelectObj.material);
                        }                
                    }
                    else{
                        if(flag){
