...
$(function () {
    ...
    function sendMessage(data){
        if(window.sendMessage && window.sendMessage instanceof Function){
            window.sendMessage(data);
        }
    }

    function contentFiltering(data,callback){
        if(window.contentFiltering && window.contentFiltering instanceof Function){
            window.contentFiltering(data,callback);
        }
    }
    ...
    var viewer = new Viewer('mainContainer',
        ...
        {
            //开发者参数
            developerID:Api.developer.developerID,
            apiUrl:Api.developer.apiUrl,//鉴权接口路径
            appID:Api.developer.appID,
            secretKey:Api.developer.secretKey,
        },
        //plugins
        {
            MatOpPlugin:{}
    ...
    function initUI(){    
        ...
        $('.productsList li').on(startEventType, function(){            
            ...
            if(index !== curShellIdx){  
                ...
                sendMessage({type:'CHANGESHELL', data:index});
                if(index === 0){
                    $('#liCenter').hide();
                    $('#liLeft').show();
                    $('#liRight').show();
        ...
        $('.colorList li').on(startEventType, function(){
            ...
            if(index !== curColorIdx){ 
                $(this).addClass('active').siblings().removeClass('active');
                sendMessage({type:'CHANGECOLOR', data:colors[index].getHexString()});
                curColorIdx = index;
        ...
        $('#iptLeftText').on('input',function(){
            ...
            contentFiltering(txt,function(result){
                document.getElementById("iptLeftText").value=result;
                $('#liLeft span').text(result.length);
                sendMessage({type:'CHANGEINPUT', data:{pos:'LEFT',text:result}});
                manageText(0, result);
            });
        });
        ...
        $('#iptRightText').on('input',function(){
            ...
            contentFiltering(txt,function(result){
                document.getElementById("iptRightText").value=result;
                $('#liRight span').text(result.length);
                sendMessage({type:'CHANGEINPUT', data:{pos:'RIGHT',text:result}});
                manageText(1, result);
            });
        });
        ...
        $('#iptCenterText').on('input',function(){
            ...
            contentFiltering(txt,function(result){
                document.getElementById("iptCenterText").value=result;
                $('#liCenter span').text(result.length);
                sendMessage({type:'CHANGEINPUT', data:{pos:'CENTER',text:result}});
                ...
                for(i; i < len; i++){
                    if(result[i] !== ' '){
                        var textGeo = genTextGeometry(result[i]);
                        var textMesh = new THREE.Mesh(textGeo, fontMat);
                        textMesh.name = result[i];
                        textMesh.scale.multiplyScalar(0.00025);
